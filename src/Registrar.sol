...
contract Registrar is ERC721URIStorage, Ownable {
    ...
    event AssetTransferred(address indexed previousOwner, address indexed newOwner, uint256 indexed _property_RegId, uint256 tokenId);
    event AssetVerified(address indexed p_owner, uint256 indexed tokenId, uint256 indexed property_RegId, uint256 survey_zip_code, uint256 survey_number, uint256 value, bool verified);
    event AssetVerificationRequest(address indexed p_owner, uint256 indexed property_RegId, uint256 survey_zip_code, uint256 survey_number, bool existed);
    ...
    function generateRwa(address _rwaOwner, uint256 _property_RegId, string memory _tokenURI, uint256 _value)
        public
        onlyOwner
        ...
        returns (uint256)
    {
        ...
        // Emit event
        emit AssetVerified(_rwaOwner, _tokenIds, _property_RegId, assets[_rwaOwner][_property_RegId].survey_zip_code, assets[_rwaOwner][_property_RegId].survey_number,_value ,true );
        _tokenIds++;
        return (_tokenIds - 1);
    }
        ...
        returns (uint256)
    {
        ...
        _mint(_rwaOwner, _tokenIds);
        _setTokenURI(_tokenIds, _tokenURI);
        emit AssetVerified(_rwaOwner, _tokenIds, _property_RegId, _survey_zip_code, _survey_number, _value ,true );
    ...
    function verification_request(
        ...
        returns (bool)
    {
        assets[_p_owner][_property_RegId] = RwaVerificationRequest(false, true, _p_owner, _property_RegId, _survey_zip_code, _survey_number, 0, 0);
        emit AssetVerificationRequest(_p_owner, _property_RegId, _survey_zip_code, _survey_number, true);
        return true;
    }


    function transferAsset(uint256 _tokenId, address _newOwner, uint256 _property_RegId) external {
...
// 20, 1000000, 123, 100112, 133, house, los angeles, 100000
